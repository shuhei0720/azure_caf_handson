# 第 9 章：Identity & Access Management（IAM）設計（2 日目）
...
## 9.1 Azure AD (Microsoft Entra ID) の理解
...
### 9.4.1 Management Group レベルのロール割り当て
...
### 9.4.2 Subscription レベルのロール割り当て
...
## 9.7 Privileged Identity Management (PIM)
...
git commit -m "Day 2: Identity and Access Management
...
ID とアクセス管理の設計が完了したら、次はガバナンス・ポリシーの実装に進みます。

👉 [第 10 章：ガバナンス・ポリシー実装（2 日目）](chapter10-governance.md)
